		...
		if(is_array($mod))
		{
			...
			foreach($mod as $dado)
			{
				...
				if(is_array($faixas) && count($faixas) > 0)
				{
					...
					if($encontrou)
				    {
						echo "<input type='checkbox' class='form-check-input' name = 'modalidade[]' value='{$dado->idmodalidade}' checked>";
					}
					else
					{
						echo "<input type='checkbox' class='form-check-input' name = 'modalidade[]' value='{$dado->idmodalidade}'>";
					}
					...
					if(!$encontrou)
					{
						echo "<option value='0'>Escolha a Faixa do {$dado->descritivo} </option>";
					}
					...
					foreach($faixas as $faixa)
					{
						...
						{
							echo "<option value='{$faixa->idfaixa}' selected>{$faixa->descritivo}</option>";
						}
						else
						{
